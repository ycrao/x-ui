...
# git mirror host
# if you don't want to use keep it to empty string
# git_mirror_host=''
# git_mirror_host='https://mirror.ghproxy.com/'
git_mirror_host='https://gh.api.99988866.xyz/'
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-${arch}.tar.gz ${git_mirror_host}https://github.com/FranzKafkaYu/x-ui/releases/download/${last_version}/x-ui-linux-${arch}.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="${git_mirror_host}https://github.com/FranzKafkaYu/x-ui/releases/download/${last_version}/x-ui-linux-${arch}.tar.gz"
    ...
    wget --no-check-certificate -O /usr/bin/x-ui ${git_mirror_host}https://raw.githubusercontent.com/FranzKafkaYu/x-ui/main/x-ui.sh
